...
/**
 * This service represents WebRTC layer of files transfer. It is responsible for:
 * - signaling during WebRTC negotiation phase,
 * - files transfer via WebRTC data channel
 * - storing active peers
 * - setting up initial WebRTC information
 */
@Injectable({
	providedIn: 'root',
})
export class WebRTCService {
	...
	public init() {
		this.api.getWebRTCConfig().then((webRTCConfig: WebRTCConfig) => {
			this.webRTCConfig = webRTCConfig;
		});

		this.ws.event$
			.pipe(filter((message) => this.isSignalingEvent(message.type)))
	...
	public sendFile(userId: string, file: File): void {
		...
		peer.createDataChannel(fileInfo).then((dataChannel) => {
	...
	private createPeer(userId: string): WebRTCPeer {
		...
		peer.onNegotiationNeeded = () => {
			peer.generateSDPOffer().then((offer) => {
	...
	private onSdpAnswer(event: SocketMessage): void {
		const peer = event.from ? this.peers[event.from] : null;
		if (peer) {
			peer.setRemoteSDP(event.message).catch(console.error);
		}
	}

	private signal(
		userId: string,
		type: WebRTCSignalingEvent,
	...
	private isSignalingEvent(event: string): event is WebRTCSignalingEvent {
		return event === 'ICE-CANDIDATE' || event === 'SDP-OFFER' || event === 'SDP-ANSWER';
	}
}
